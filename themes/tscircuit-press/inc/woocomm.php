...
// 	End add multiple products
  ...
  //woocommerce wrappers
	...
	function tscircuit_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12">';
	}
	function tscircuit_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'tscircuit_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'tscircuit_wrapper_end', 10);
	...
	function tscircuit_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'tscircuit_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function tscircuit_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'tscircuit_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function tscircuit_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'tscircuit_place_order_button_text' );
  ...
  //change add to cart button text
	function tscircuit_woocommerce_product_add_to_cart_text(){
		return 'Register Now';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'tscircuit_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'tscircuit_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'tscircuit_woocommerce_product_add_to_cart_text' );
...
function tscircuit_wc_scripts_styles() {
	if( is_admin() ) return;
...
add_action( 'wp_enqueue_scripts', 'tscircuit_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/tscircuit-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function tscircuit_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'dcp_team_info_lower_level', 'dcp_team_info', 'dcp_team_info_act_ii', 'event' );
        ...
        if( in_array($product_cat_slug, $extra_types) ) {
          switch( $product_cat_slug ) {
            ...
            case 'gear':
              ...
              case 'dcp_team_info_lower_level':
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  'title'       => $product_name,
                  'type'        => 'text',
                ...
                $extra_fields[] = array(
                  ...
                  'options'     => array(
                            ''    => 'Requested Division',
                            '10U/4th Grade' => "10U/4th Grade",
                            '11U/5th Grade' => "11U/5th Grade",
                            '12U/6th Grade' => "12U/6th Grade",
                            '13U/7th Grade' => "13U/7th Grade",
                            '14U/8th Grade' => "14U/8th Grade",
                  )
                );
                $extra_fields[] = array(
                  'title'       => $product_name,
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'options'     => array(
                            ''    => 'Requested Division',
                            '15U/Frosh/Soph' => "15U/Frosh/Soph",
                            '16U/JV' => "16U/JV",
                            '17U/Varsity' => "17U/Varsity",
                  )
                );
                $extra_fields[] = array(
                  'title'       => $product_name,
...
add_filter( 'woocommerce_before_cart' , 'tscircuit_cart_setup');
function tscircuit_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = tscircuit_cart_process();
  ...
  if(!empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return tscircuit_player_checkout_field( $field, $order_data_field ); });
  }
}
...
function tscircuit_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  echo '<input type="hidden" name="tscircuit_extra_data_required" value="' . implode(',',$added_field_names_required) . '" />';
  echo '<input type="hidden" name="tscircuit_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'tscircuit_player_checkout_field_process');
function tscircuit_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['tscircuit_extra_data_required'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['tscircuit_extra_data_required']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'tscircuit_player_checkout_field_update_order_meta' );
function tscircuit_player_checkout_field_update_order_meta( $order_id ) {
  //join the extra_data list if we need to
  $tscircuit_extra_data = array();
  $tscircuit_extra_data_fields = array();
  if ( $_POST['tscircuit_extra_data'] ) {
    $tscircuit_extra_data[] = $_POST['tscircuit_extra_data'];
    $tscircuit_extra_data_fields[] = $_POST['tscircuit_extra_data'];
  }
  if ( $_POST['tscircuit_extra_data_required'] ) {
    $tscircuit_extra_data[] = $_POST['tscircuit_extra_data_required'];
    $tscircuit_extra_data_fields[] = $_POST['tscircuit_extra_data_required'];
  }
  //save the field reference so we know what to pull later
  if ( !empty($tscircuit_extra_data) ) update_post_meta( $order_id, 'tscircuit_extra_data', sanitize_text_field( implode(',', $tscircuit_extra_data) ) );
  // Check if we have extra data
  if ( !empty($tscircuit_extra_data_fields) ) {
    //parse extra field names, then loop and save
    $fields = explode(',', implode(',', $tscircuit_extra_data_fields));
    foreach( $fields as $dex => $key ) {
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'tscircuit_player_checkout_field_display_admin_order_meta', 10, 1 );
function tscircuit_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'tscircuit_extra_data', true );
  if( !empty($fields) ) {
    $fields = explode(',', $fields);
    //     print_r($fields);
    
    // Move elements containing "select_findus" to the first position
    $findusElements = array_filter($fields, function ($element) {
        return strpos($element, "select_findus") !== false;
    });

    $otherElements = array_diff($fields, $findusElements);

    // Sort the remaining elements based on the order "1_0, 1_1, 1_2"
    usort($otherElements, function ($a, $b) {
        preg_match('/_(\d+)_(\d+)$/', $a, $matchesA);
        preg_match('/_(\d+)_(\d+)$/', $b, $matchesB);

        $firstComparison = $matchesA[1] - $matchesB[1];
    ...
    $fields = array_merge($findusElements, $otherElements);
//     print_r($fields);
...
function tscircuit_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'tscircuit_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'tscircuit_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(tscircuit_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = tscircuit_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
add_filter( 'woocommerce_checkout_fields' , 'tscircuit_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function tscircuit_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to tscircuit about the purchase
add_action('woocommerce_after_checkout_validation', 'process_tscircuit_data');
function process_tscircuit_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'tscircuit_update_meta_form_fields' );
function tscircuit_update_meta_form_fields( $order_id ) {
  if ( ! empty( $_POST['order_data'] ) ) update_post_meta( $order_id, '_order_data', $_POST['order_data'] );
}

//display and custom order data for admin
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'tscircuit_checkout_field_display_admin_order_meta', 10, 1 );
function tscircuit_checkout_field_display_admin_order_meta($order){
...
// function OLD_tscircuit_checkout_field_display_admin_order_meta($order){
//   //for setting form restrictions/helpers
//   $tscircuit_cart_items = array();
//   //return string
//   $to_write = '';
...
//       // update functionality
//       //
//   wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/tscircuit-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
// 	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
//   wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
...
add_action( 'show_user_profile', 'tscircuit_user_fields_display' );
add_action( 'edit_user_profile', 'tscircuit_user_fields_display' );

function tscircuit_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/tscircuit-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'tscircuit' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'tscircuit_user_field_error', 10, 3 );
function tscircuit_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'tscircuit_user_fields_update' );
add_action( 'edit_user_profile_update', 'tscircuit_user_fields_update' );

function tscircuit_user_fields_update( $user_id ) {
...
//My account add custom editor sections
add_filter('woocommerce_account_menu_items', 'tscircuit_account_customization');
function tscircuit_account_customization($menu_links){
  if(current_user_can('front_editor') || current_user_can('administrator')){
    if( current_user_can('coach_member') || current_user_can('administrator') ){ $profile_links[ 'tscircuit-home' ] = 'Digital Coach Packets'; }
    $menu_links = array_slice( $menu_links, 0, 1, true ) + $profile_links + array_slice( $menu_links, 1, NULL, true );
  }
  return $menu_links;
}
//Add account data edit
add_action('init', 'tscircuit_data_edit_endpoint');
function tscircuit_data_edit_endpoint(){
  add_rewrite_endpoint( 'tscircuit-home', EP_PAGES );
}
//My account edit data page
add_action('woocommerce_account_tscircuit-home_endpoint', 'tscircuit_data_endpoint_content');
function tscircuit_data_endpoint_content(){
  tscircuit_dir_render('digital-coach-packets', 'home', get_current_user_id(), $arg = null);
}
?>
<?php
