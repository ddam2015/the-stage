...
var tscircuit_reveal_closer_today_button = $('#reveal_close_today');
if (tscircuit_reveal_closer_today_button.length > 0) {
  tscircuit_reveal_closer_today_button.on('click', function() {
    localStorage.setItem("tscircuit_close_today", 'true');
    localStorage.setItem("tscircuit_close_today_date", new Date() );
  });
}
...
var statsSubmenu = $('.stats-menu ul');

$('#main-nav li').hover(function(e) {
  if(e.currentTarget.classList.contains('stats-menu')) {
    statsSubmenu.css('display','block');
  } else {
    statsSubmenu.css('display','none');
  }
})

// event menu shorten locations
  // first evaluate columns
function abbreviateMenuLocations() {
  var events = document.querySelectorAll('.event-menu .logo-img');

  events.forEach(function(event){
    var currentEvent = event.querySelector('small');
    var innerText = event.querySelector('small').innerHTML;
//     split text by <br> which separates date from location
    var splitText = innerText.split('<br>');
    
    console.log(splitText[0], splitText[1]);
    
   var strIndex = splitText[1].indexOf(',');
    
     if(strIndex == -1) {
           //string not found - single location do nothing
       console.log('single')
         return;
        } else {
          //string found - multiple location - split string to only first and add + icon
          var commaText = splitText[1].split(',');
          console.log(commaText);
          if(commaText.length > 2) {
            var newText = commaText[0] + ',' + commaText[1] + '<span class="additional-info__plus">+<small class="additional-info__plus-tooltip">Event at multiple locations</small></span>';
            currentEvent.innerHTML = splitText[0] + '<br>' + newText;
          }
...
abbreviateMenuLocations();

$('.event-menu .menu').append('<p class="event__facility-key"><span class="additional-info__plus">+</span> Event is hosted at multiple locations</p>');
