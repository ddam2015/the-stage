<?php $select_year = $_POST['g365_year']; $select_group = $_POST['group_lv']; $level_of_play = $_POST['lv_of_play'];
  if(!isset($select_year) && !isset($select_group)){$select_year = wp_date('Y'); $select_group = '17U';}
  $g365_team_standings = g365_fn(['fn_name'=>'g365_team_standing', 'arguments'=>[['post_year'=>$select_year, 'post_gp_lv'=>$select_group, 'is_year'=>$is_year, 'is_gp_lv'=>$is_gp_lv, 'is_dcp_only'=>true, 'post_ros_dvs'=>$level_of_play]]]); $g365_team_standings = json_decode( json_encode($g365_team_standings), true);
  echo $g365_team_standings[0]; if(!empty($g365_team_standings[1])): /*main*/ foreach($g365_team_standings[1] as $level_index => $club_team_data): /*foreach-main*/?>
  <div id="dialong_div"></div>
  <div class="max-width-1200">
    <h5><?php echo $g365_team_standings[3][$level_index]. ' ' .$level_of_play; ?></h5>
                    ...
                    if(!empty($boxscore_list)): //c
                      ...
                      <div class="stats_customize cts_res flex items-center small-margin-bottom small-12 medium-12 large-12">
                        ...
                        <div class="opp_logo_box hide-for-small-only">
                          <a href="<?php echo $g365_team_standings[5]['g365_url'].'/club/'.$boxscore_list['opp_nickname'].'/teams'; ?>" target="_blank"><img style="height:100px;width:125px;" alt="<?php echo $boxscore_list['full_team_name']; ?>" title="<?php echo $boxscore_list['full_team_name']; ?>" src="<?php echo (!empty($boxscore_list['opp_logo']) ? $boxscore_list['opp_logo'] != "NULL" ? $g365_team_standings[5]['org_logo'].$boxscore_list['opp_logo'] : $g365_team_standings[5]['placeholder_img'] : $g365_team_standings[5]['placeholder_img']); ?>"></a>
                        </div>
                      </div>
